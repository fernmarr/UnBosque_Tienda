		...
		function loadusuarios() {
			...
			xmlhttp.onreadystatechange = function() {
				if (xmlhttp.readyState === 4 && xmlhttp.status === 200) {
					...
					var tbltop = "<table class='table table-striped table-dark'><thead><tr><th scope='col'>C&eacute;dula</th><th scope='col'>Email</th><th scope='col'>Nombre</th><th scope='col'>Usuario</th></tr></thead><tbody>";
					var main = "";
					for (i = 0; i < usuarios.length; i++) {
		...
		function consultarUsuario() {
				...
				req.open('GET', 'http://localhost:8080/consultarusuario?cedula_usuario='+cedula_usuarioA, false);
				req.send(null);
				var usuario = null;
				if (req.status == 200)
		...
		function actualizarUsuario() {
			...
			var y = document.getElementById("cedula_usuarioA").value;
			var req = new XMLHttpRequest();
			var coincidencia = false;
			req.open('GET', 'http://localhost:8080/listarusuarios', false);
			req.send(null);
			...
			if (coincidencia != false) {
				...
				var element = document.getElementById("errorA");
				element.classList.remove("d-none");
				var element2 = document.getElementById("okCreaA");
				element2.classList.add("d-none");
				var element3 = document.getElementById("btnA");
		...
		function loadusuariosBorrar() {
			...
			xmlhttp.onreadystatechange = function() {
				if (xmlhttp.readyState === 4 && xmlhttp.status === 200) {
					var usuarios = JSON.parse(xmlhttp.responseText);
					var tbltop = "<table class='table table-striped table-dark'><thead><tr><th scope='col'>C&eacute;dula</th><th scope='col'>Email</th><th scope='col'>Nombre</th><th scope='col'>Usuario</th><th scope='col'>Borrar</th></tr></thead><tbody>";
					var main = "";
					for (i = 0; i < usuarios.length; i++) {
						main += "<tr><td>" + usuarios[i].cedula_usuario
								+ "</td><td>" + usuarios[i].email_usuario
